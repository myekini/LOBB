  ...
  return (
    <div ref={menuRef} className="relative hidden md:block">
      <LobbButton variant="unstyled"
        ...
        className="flex h-10 items-center gap-2 rounded-[var(--lobb-radius-lg)] border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] py-1 pl-1 pr-2 text-[var(--lobb-text-primary)] shadow-[var(--lobb-shadow-card)] transition hover:border-[var(--lobb-clay)]/40"
      >
        <span className="flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-[var(--lobb-radius-md)] bg-[var(--lobb-clay)] text-white text-[11px] font-medium">
          {profile?.avatar_url ? (
        ...
        <span className="hidden max-w-20 truncate text-xs font-medium md:block">{firstName}</span>
        <ChevronDown className="size-3.5 text-[var(--lobb-text-tertiary)]" />
      </LobbButton>

      {open && (
        <div className="absolute right-0 top-[calc(100%+10px)] z-50 w-64 overflow-hidden rounded-[var(--lobb-radius-lg)] border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] p-2 shadow-[var(--lobb-shadow-modal)]">
          <div className="flex items-center gap-3 border-b border-[var(--lobb-border-subtle)] p-3">
            <span className="flex size-11 shrink-0 items-center justify-center overflow-hidden rounded-[var(--lobb-radius-lg)] bg-[var(--lobb-clay)] text-white text-sm font-medium">
              {profile?.avatar_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={profile.avatar_url} alt="" className="size-full object-cover" />
              ) : initials ? (
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-medium">{profile?.full_name ?? "Player"}</p>
              <p className="truncate text-[11px] font-medium text-[var(--lobb-text-secondary)]">{profile?.email ?? "LOBB player"}</p>
            </div>
          </div>
          ...
          <Link
            ...
            className="flex h-11 items-center gap-3 rounded-[var(--lobb-radius-lg)] px-3 text-sm font-medium text-[var(--lobb-text-primary)] transition hover:bg-[var(--lobb-bg-secondary)]"
          >
          ...
          <LobbButton variant="unstyled"
            ...
            onClick={logout}
            disabled={busy}
            className="mt-1 flex h-11 w-full items-center gap-3 rounded-[var(--lobb-radius-lg)] px-3 text-left text-sm font-medium text-[var(--lobb-error)] transition hover:bg-[var(--lobb-bg-secondary)] disabled:opacity-60"
          >
      ...
      <div className="mx-auto grid h-16 max-w-6xl grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 sm:px-6 md:flex md:justify-between">
        <div className="flex min-w-0 items-center gap-2">
          <Link
            ...
            className="lobb-logo-shell flex size-10 shrink-0 items-center justify-center overflow-hidden rounded-[var(--lobb-radius-md)]"
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          {backHref && (
            <Link
              href={backHref}
              aria-label="Go back"
              className="flex size-10 shrink-0 items-center justify-center rounded-[var(--lobb-radius-md)] border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] text-[var(--lobb-text-primary)] transition hover:border-[var(--lobb-clay)]/40 active:scale-[0.97]"
            >
          ...
          <div className="hidden min-w-0 md:block">
            {eyebrow && (
              <p className="truncate text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--lobb-clay)]">
                {eyebrow}
              </p>
            )}
            <p className="truncate text-sm font-medium text-[var(--lobb-text-primary)]">{title}</p>
          </div>
        </div>
        ...
        <div className="min-w-0 text-center md:hidden">
          ...
          <h1 className="truncate text-[15px] font-semibold text-[var(--lobb-text-primary)]">{title}</h1>
        </div>
