...
export function BookingCard({ booking, href }: { booking: DashboardBooking; href: string }) {
  return (
    <article className="lobb-app-card border border-[var(--lobb-border)] bg-[var(--lobb-surface)] p-4">
      ...
      <Link href={href} className="mt-4 flex h-10 items-center justify-center rounded-[12px] border border-[var(--lobb-border)] text-xs font-black transition hover:border-[var(--lobb-clay)]/40 hover:text-[var(--lobb-clay)]">
        View details
      </Link>
    </article>
  );
