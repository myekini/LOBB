    ...
    >
      <Combobox.Trigger
        ...
        className={cn(
          ...
          invalid ? "border-[var(--lobb-error)]/50" : "border-[var(--lobb-border-subtle)]",
          className
        )}
      >
        <span className={cn("truncate", !selected && "font-medium text-[var(--lobb-text-tertiary)]")}>
      ...
      <Combobox.Portal>
        <Combobox.Positioner side="bottom" sideOffset={6} className="isolate z-[90] w-[var(--anchor-width)]">
          <Combobox.Popup className="max-h-[min(340px,var(--available-height))] w-full overflow-hidden rounded-[var(--lobb-radius-lg)] border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] shadow-[var(--lobb-shadow-modal)]">
            <div className="flex items-center gap-2.5 border-b border-[var(--lobb-border-subtle)] px-4">
              <Search className="size-4 shrink-0 text-[var(--lobb-text-tertiary)]" />
              <Combobox.Input
                placeholder={searchPlaceholder}
                className="h-12 w-full border-0 bg-transparent text-[14px] font-medium text-[var(--lobb-text-primary)] outline-none placeholder:text-[var(--lobb-text-tertiary)] focus:ring-0"
              />
            </div>
            <Combobox.Empty className="px-4 py-5 text-center text-[13px] font-medium text-[var(--lobb-text-tertiary)] empty:hidden">
              {emptyMessage}
            </Combobox.Empty>
            <Combobox.List className="max-h-[260px] overflow-y-auto p-1.5">
              {(item: { value: string; label: string }) => (
                <Combobox.Item
                  key={item.value}
                  value={item}
                  className="flex cursor-pointer items-center justify-between gap-3 rounded-[var(--lobb-radius-md)] px-3.5 py-2.5 text-[14px] font-medium text-[var(--lobb-text-secondary)] transition-colors data-[highlighted]:bg-[var(--lobb-clay)]/10 data-[highlighted]:text-[var(--lobb-text-primary)] data-[selected]:text-[var(--lobb-clay)]"
                >
