  ...
  return (
    <main className="lobb-app-page min-h-screen pb-28 text-[var(--lobb-text-primary)]">
      ...
      <div className="mx-auto max-w-lg px-5 pt-6 sm:px-6">
        ...
        {/* Status card */}
        {loading ? (
          <SkeletonBlock className="h-24 w-full rounded-[var(--lobb-radius-lg)]" />
        ) : meta && (
          <div className="lobb-surface-outlined mb-5 flex items-start gap-4 border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] p-5">
            <div
              className="flex size-12 shrink-0 items-center justify-center rounded-[var(--lobb-radius-lg)]"
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-[15px] font-medium" style={{ color: meta.color }}>
                {meta.label}
              </p>
              {kycState?.kyc_failed_reason && (
                <p className="mt-1 text-xs font-medium text-[var(--lobb-error)]">
                  {kycState.kyc_failed_reason}
                </p>
              )}
            </div>
        ...
        {!loading && kycState && isVerified(kycState.kyc_status) && (
          <div className="lobb-surface-outlined border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] p-6 text-center">
            <CheckCircle2 className="mx-auto size-10 text-[var(--lobb-success)]" />
            <p className="mt-3 text-lg font-medium">Identity verified</p>
            ...
            <LobbButton variant="unstyled"
              ...
              className="mt-5 inline-flex h-11 items-center rounded-[var(--lobb-radius-md)] bg-[var(--lobb-bg-inverse)] px-5 text-sm font-medium text-[var(--lobb-text-inverse)]"
            >
              Set up payout bank
            </LobbButton>
          </div>
        ...
        {!loading && kycState && isReviewing(kycState.kyc_status) && !success && (
          <div className="lobb-surface-outlined border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] p-6 text-center">
            ...
            <p className="mt-3 text-lg font-medium">Verification in progress</p>
            <p className="mt-1 text-sm font-medium leading-6 text-[var(--lobb-text-secondary)]">
              Your details have been submitted. We&apos;ll notify you once verification is complete — usually within 24 hours.
            </p>
            <p className="mt-4 text-xs font-medium text-[var(--lobb-text-tertiary)]">
        ...
        {success && (
          <div className="mb-5 rounded-[var(--lobb-radius-lg)] bg-[var(--lobb-success-soft)] px-4 py-3">
            <p className="text-sm font-medium text-[var(--lobb-success)]">Details submitted successfully.</p>
            <p className="mt-0.5 text-xs font-medium text-[var(--lobb-success)]">
        ...
        {!loading && kycState && (!isVerified(kycState.kyc_status) && !isReviewing(kycState.kyc_status)) && (
          <form onSubmit={submit} className="lobb-surface-outlined space-y-5 border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] p-5">
            <div>
            ...
            <div>
              ...
              <p className="mt-1.5 text-[11px] font-medium text-[var(--lobb-text-tertiary)]">
        ...
        {/* What happens next */}
        {!loading && kycState && !isVerified(kycState.kyc_status) && (
          <div className="mt-4 space-y-2 rounded-[var(--lobb-radius-lg)] border border-[var(--lobb-border-subtle)] bg-[var(--lobb-bg-elevated)] p-4">
            <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--lobb-text-tertiary)]">
              What happens next
            </p>
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <span className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-[var(--lobb-clay-light)] text-[10px] font-medium text-[var(--lobb-clay)]">
                  {i + 1}
                </span>
                <p className="text-xs font-medium leading-5 text-[var(--lobb-text-secondary)]">{step}</p>
              </div>
            ))}
          </div>
        )}
